...
// Adding Event Listeners Desktop_________________________
...
canvas.addEventListener('touchstart', e => {
  drawInfo(`Touch Start  X ${Math.floor(e.touches[0].clientX)} Y ${Math.floor(e.touches[0].clientY)}`);
  mouseDown(Math.floor(e.touches[0].clientX), Math.floor(e.touches[0].clientY));

  canvas.addEventListener('touchmove', e => {
    if (isAnimation === false) {
      drawInfo(`Touch Move  X ${Math.floor(e.touches[0].clientX)} Y ${Math.floor(e.touches[0].clientY)}`);
      mouseMove(Math.floor(e.touches[0].clientX), Math.floor(e.touches[0].clientY));
    };
  }, false);

  canvas.addEventListener('touchend', e => {
    drawInfo(`Touch End  X ${Math.floor(e.touches[0].clientX)} Y ${Math.floor(e.touches[0].clientY)}`);
    mouseUp(Math.floor(e.touches[0].clientX), Math.floor(e.touches[0].clientY));
  }, false);
...
// Event Functions______________________________
...
function mouseUp (x, y) {
  ...
  if (activeBlocks === true & activeFeatures === true) { // Create New Element
    doFormatFeature(hitIndex);
    createLinkOrBlock(x, y);
  };
  if (oneClickX == x & oneClickY == y) {
...
function keyUp (key) {
  if (key == 'Delete' & activeBlocks === true) {
    deleteElement();
  };
  if (key == 'Enter') {
    deleteInput();
