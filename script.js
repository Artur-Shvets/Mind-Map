...
canvas.addEventListener('touchstart', e => {
  drawInfo(`Touch Start  X ${Math.round(e.touches[0].clientX)} Y ${Math.round(e.touches[0].clientY)}`);
  mouseDown(Math.round(e.touches[0].clientX), Math.round(e.touches[0].clientY));

  canvas.addEventListener('touchmove', e => {
    if (isAnimation === false) {
      mouseMove(Math.round(e.touches[0].clientX), Math.round(e.touches[0].clientY));
      drawInfo(`Touch Move  X ${Math.round(e.touches[0].clientX)} Y ${Math.floor(e.touches[0].clientY)}`);
    };
  }, false);

  canvas.addEventListener('touchend', e => {
    mouseUp(Math.round(e.touches[0].clientX), Math.round(e.touches[0].clientY));
    drawInfo(`Touch End  X ${Math.round(e.touches[0].clientX)} Y ${Math.round(e.touches[0].clientY)}`);
  }, false);
